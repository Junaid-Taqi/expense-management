...
import { addCategory, deleteCategory, updateCategory, selectCategories, fetchCategories } from '../store/slices/categorySlice';
import { selectAllExpenses, fetchExpenses } from '../store/slices/expenseSlice';
import { MdAdd, MdDelete, MdCategory, MdEdit, MdClose, MdSave } from 'react-icons/md';
...
const ManageCategories = () => {
  ...
  const handleUpdate = (e) => {
    ...
    if (categories.some(c => c.name.toLowerCase() === cleanName.toLowerCase() && c._id !== editingId)) {
      ...
      <div className="row">
        <div className="col-lg-8">
          <div className="stat-card p-4">
            ...
            <form onSubmit={editingId ? handleUpdate : handleAdd} className="mb-4 d-flex gap-2">
              ...
              {editingId ? (
                <>
                  <button type="submit" className="btn btn-success d-flex align-items-center px-4 rounded-2">
                    <MdSave className="me-1" /> Save
                  </button>
                  <button type="button" onClick={cancelEdit} className="btn btn-outline-secondary d-flex align-items-center px-4 rounded-2">
            ...
            <h5 className="fw-bold mb-3 mt-5">Category Management List</h5>
                    ...
                    <tr key={cat._id}>
                      ...
                      <td>
                        <span className="badge bg-light text-dark border">
                          <MdCategory className="me-1" /> {cat.icon || 'category'}
                        </span>
                      </td>
            ...
            {categories.length === 0 && (
              <p className="text-muted text-center py-4">No custom categories found. Add one above!</p>
            )}
